...
Результат оптимизаций: indvars: индуктивные переменные были упрощены, licn: инвариантные вычисления вынесены из циклов, loop-unroll-full: цикл был полностью развёрнут — итерации цикла заменены на повторяющиеся блоки кода без самого цикла.
...
Первая оптимизация - преобразование while в for (каноническая форма)
Преобразует цикл while в каноническую форму for, что явно выделяет инициализацию, условие и шаг, упрощая вывод диапазона изменения индекса и предсказание числа итераций. Также оптимизация позволяет компилятору легче применять стандартные методы анализа и оптимизации. 

Вторая оптимизация - перевод в SSA-форму
Приводит код к виду, где каждая переменная получает значение ровно один раз, вводя φ-функции в точках слияния потоков, что упрощает анализ зависимостей, устранение мертвого кода и постоянных выражений.
...
Блок-схема перевода цикла while в for

![](./dop/while.png)

Блок-схема перевода инкремента в SSA
![](./dop/ssa.png)
